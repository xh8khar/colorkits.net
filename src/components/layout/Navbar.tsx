...
const toolCategories = navItems.filter(item => item.children)
...
function ToolsDropdown() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute left-0 top-full mt-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl py-1 min-w-[200px] z-50 animate-fade-in">
          {toolCategories.map(cat => (
            <Link
              key={cat.label}
              href={cat.children![0].href}
...
function SocialShare() {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="p-2 rounded-lg text-slate-500 dark:text-slate-400 hover:text-rose-500 dark:hover:text-rose-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer"
